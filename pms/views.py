...
class BookingSearchView(View):
    ...
    def get(self, request):
        ...
        bookings = Booking.objects.filter(
            Q(code__icontains=query["filter"])
...
class RoomSearchView(View):
    ...
    def get(self, request):
        room_search_form = RoomSearchForm()
        context = {"form": room_search_form, "menu_reserva": "active btn btn-primary"}
...
class HomeView(View):
    ...
    def get(self, request):
        ...
        context = {"bookings": bookings, "menu_home": "active btn btn-primary"}
        return render(request, "home.html", context)
...
class DashboardView(View):
    def get(self, request):
        ...
        new_bookings = (
            Booking.objects.filter(created__range=today_range).values("id")
        ).count()

        # get incoming guests
        incoming = (
            Booking.objects.filter(checkin=today).exclude(state="DEL").values("id")
        ).count()

        # get outcoming guests
        outcoming = (
            Booking.objects.filter(checkout=today).exclude(state="DEL").values("id")
        ).count()

        # get outcoming guests
        invoiced = (
            Booking.objects.filter(created__range=today_range)
            .exclude(state="DEL")
            .aggregate(Sum("total"))
        )
